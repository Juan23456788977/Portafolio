<?php
// filepath: c:\xampp\htdocs\O\subirFotoSupervisor.php
require 'db.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['foto']) && isset($_POST['email'])) {
    ...
    $nombreArchivo = 'fotos/supervisor_' . md5($email . time()) . '.' . $ext;
    if (!is_dir('fotos')) mkdir('fotos');
    if (move_uploaded_file($file['tmp_name'], $nombreArchivo)) {
        $stmt = $pdo->prepare("UPDATE supervisores SET foto=? WHERE email=?");
